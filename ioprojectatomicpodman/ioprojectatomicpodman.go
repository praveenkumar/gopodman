...
type BuildInfo struct {
	...
	Tags          []string          `json:"tags"`
	Add_hosts     []string          `json:"add_hosts"`
	Cgroup_parent string            `json:"cgroup_parent"`
	Cpu_period    int64             `json:"cpu_period"`
	Cpu_quota     int64             `json:"cpu_quota"`
	Cpu_shares    int64             `json:"cpu_shares"`
	...
	Memory_swap   string            `json:"memory_swap"`
	Security_opts []string          `json:"security_opts"`
	Shm_size      string            `json:"shm_size"`
	Ulimit        []string          `json:"ulimit"`
	Volume        []string          `json:"volume"`
	Squash        bool              `json:"squash"`
	Pull          bool              `json:"pull"`
	Pull_always   bool              `json:"pull_always"`
	Force_rm      bool              `json:"force_rm"`
	Rm            bool              `json:"rm"`
	Label         []string          `json:"label"`
	Annotations   []string          `json:"annotations"`
	Build_args    map[string]string `json:"build_args"`
...
func (m GetImage_methods) Call(c *varlink.Connection, name_in_ string) (image_out_ ImageInList, err_ error) {
	receive, err_ := m.Send(c, 0, name_in_)
	if err_ != nil {
		return
	}
...
func (m GetImage_methods) Send(c *varlink.Connection, flags uint64, name_in_ string) (func() (ImageInList, uint64, error), error) {
	var in struct {
		Name string `json:"name"`
	}
	in.Name = name_in_
	receive, err := c.Send("io.projectatomic.podman.GetImage", in, flags)
	if err != nil {
		return nil, err
	}
	return func() (image_out_ ImageInList, flags uint64, err error) {
		var out struct {
			Image ImageInList `json:"image"`
		}
		flags, err = receive(&out)
		if err != nil {
			return
...
func (m BuildImage_methods) Send(c *varlink.Connection, flags uint64, build_in_ BuildInfo) (func() ([]string, uint64, error), error) {
	...
	in.Build = build_in_
	receive, err := c.Send("io.projectatomic.podman.BuildImage", in, flags)
	if err != nil {
		return nil, err
	}
...
type ioprojectatomicpodmanInterface interface {
	...
	GetImage(c VarlinkCall, name_ string) error
...
func (c *VarlinkCall) ReplyGetImage(image_ ImageInList) error {
	var out struct {
		Image ImageInList `json:"image"`
	}
	out.Image = image_
...
// GetImage returns a single image in an [ImageInList](#ImageInList) struct.  You must supply an image name as a string.
// If the image cannot be found, an [ImageNotFound](#ImageNotFound) error will be returned.
func (s *VarlinkInterface) GetImage(c VarlinkCall, name_ string) error {
	return c.ReplyMethodNotImplemented("io.projectatomic.podman.GetImage")
}
...
func (s *VarlinkInterface) VarlinkDispatch(call varlink.Call, methodname string) error {
	...
	case "GetImage":
		...
		return s.ioprojectatomicpodmanInterface.GetImage(VarlinkCall{call}, in.Name)
	...
	case "BuildImage":
		...
		return s.ioprojectatomicpodmanInterface.BuildImage(VarlinkCall{call}, in.Build)
